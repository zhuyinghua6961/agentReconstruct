...
def test_gateway_beijing_formatter_formats_epoch_with_offset():
    formatter = BeijingFormatter("%(asctime)s")
    record = logging.LogRecord("gateway.test", logging.INFO, __file__, 1, "message", (), None)
    record.created = 0

    assert formatter.formatTime(record) == "1970-01-01T08:00:00.000+08:00"


def test_gateway_app_log_format_includes_pid_and_trace_fields():
    assert APP_LOG_FORMAT == "%(asctime)s %(levelname)s [pid=%(process)d] [%(name)s] [trace=%(trace_id)s] %(message)s"


def test_gateway_gunicorn_access_log_format_is_structured():
...
def test_gateway_start_script_uses_gunicorn_config_file():
