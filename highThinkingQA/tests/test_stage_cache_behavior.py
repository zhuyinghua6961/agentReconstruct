...
def test_run_agent_reuses_cached_direct_answer_and_decompose(monkeypatch):
    ...
    assert first.error == ""
    assert second.error == ""
    assert direct_calls["n"] == 1
    assert decompose_calls["n"] == 1


def test_batch_retrieve_reuses_cached_results(monkeypatch):
...
def test_batch_retrieve_singleflights_same_query_when_redis_available(monkeypatch):
    from retriever import vector_retriever
    from server.services import stage_cache as stage_cache_module
