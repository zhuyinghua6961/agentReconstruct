...
class AdminUsersService:
    ...
    def create_user(self, *, username: str, password: str, user_type: str) -> dict[str, Any]:
        try:
            ...
            return {
                ...
                "data": {"id": created_id, "username": username, "role": "user", "user_type": user_type_code, "status": "active"},
            }
        except Exception as exc:
            if _is_db_unavailable_error(exc):
                return self._db_error(exc)
    ...
    def reset_password(self, *, target_user_id: int, actor_user_id: int, new_password: str) -> dict[str, Any]:
        try:
            ...
            validation = auth_service.validate_password_strength(password=new_password, role=str(user.get("role") or "user"))
        ...
        try:
            ...
            if current_type == target_type:
                return {
                    "success": True,
                    "message": "用户身份无变化",
                    "data": {"id": int(user["id"]), "username": user["username"], "user_type": current_type, "role": role_text or "user"},
                }
    ...
    def batch_delete_users(self, *, target_user_ids: list[Any], actor_user_id: int) -> dict[str, Any]:
        summary = {"deleted": 0, "skipped": 0, "failed": 0}
        details: list[dict[str, Any]] = []
        for raw_id in list(target_user_ids or []):
            try:
                user_id = int(raw_id)
            except Exception:
                summary["failed"] += 1
                details.append({"user_id": raw_id, "status": "failed", "reason": "invalid_user_id"})
                continue
            result = self.delete_user(target_user_id=user_id, actor_user_id=actor_user_id)
            if result.get("success"):
                summary["deleted"] += 1
                details.append({"user_id": user_id, "status": "deleted"})
            else:
                code = str(result.get("code") or "")
                if code in {"USER_NOT_FOUND", "PERMISSION_DENIED"}:
                    summary["skipped"] += 1
                    details.append({"user_id": user_id, "status": "skipped", "reason": result.get("error")})
                else:
                    summary["failed"] += 1
                    details.append({"user_id": user_id, "status": "failed", "reason": result.get("error")})
        return {"success": True, "data": {"summary": summary, "details": details}}

    def batch_change_user_type(self, *, target_user_ids: list[Any], target_type_raw: object) -> dict[str, Any]:
        summary = {"changed": 0, "skipped": 0, "failed": 0}
        details: list[dict[str, Any]] = []
        for raw_id in list(target_user_ids or []):
            try:
                user_id = int(raw_id)
                ...
                details.append({"user_id": raw_id, "status": "failed", "reason": "invalid_user_id"})
                continue
            result = self.update_type(target_user_id=user_id, target_type_raw=target_type_raw)
            if result.get("success"):
                current = result.get("data") if isinstance(result.get("data"), dict) else {}
                unchanged = str(result.get("message") or "") == "用户身份无变化"
                if unchanged:
                    summary["skipped"] += 1
                    details.append({"user_id": user_id, "status": "skipped", "user_type": current.get("user_type")})
                else:
                    summary["changed"] += 1
                    details.append({"user_id": user_id, "status": "changed", "user_type": current.get("user_type")})
            else:
                code = str(result.get("code") or "")
                if code in {"USER_NOT_FOUND", "PERMISSION_DENIED"}:
                    summary["skipped"] += 1
