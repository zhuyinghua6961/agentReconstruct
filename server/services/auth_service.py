...
def _is_db_unavailable_error(exc: Exception) -> bool:
    return isinstance(exc, (DatabaseConfigError, DatabaseConnectionError))
...
class AuthService:
    def __init__(self, *, repo: AuthRepository | None = None, token_service: TokenService | None = None) -> None:
    ...
    def _check_password_history(self, *, user_id: int, role: str, new_password: str) -> dict[str, Any]:
        ...
        for old_hash in self._repo.list_recent_password_hashes(user_id=user_id, limit=history_limit):
    ...
    def _build_user_payload(self, user: dict[str, Any]) -> dict[str, Any]:
        ...
        require_security_questions_setup = bool(user.get("must_set_security_questions", False)) and (
            not has_security_questions
        )
    ...
    def get_user_by_id(self, user_id: int) -> dict[str, Any] | None:
        return self._repo.get_by_id(int(user_id))
    ...
    def login(self, username: str, password: str) -> dict[str, Any]:
        ...
        try:
            ...
            self._repo.reset_login_attempts(user_id=int(user["id"]))
            token = self._tokens.issue_access_token(user_id=int(user["id"]), role=str(user.get("role") or "user"))
    ...
    def change_password(self, *, user_id: int, old_password: str, new_password: str) -> dict[str, Any]:
        try:
            ...
            if not user:
                return {"success": False, "error": "user_not_found", "code": "USER_NOT_FOUND"}
            if not _verify_password(str(old_password or ""), str(user.get("password_hash") or "")):
            ...
            if not validation.get("success"):
                return validation
            if _verify_password(str(new_password or ""), str(user.get("password_hash") or "")):
                return {
                    "success": False,
                    "error": f"新密码不能与最近{self._password_history_limit(role)}次使用的密码相同",
                    "code": "PASSWORD_REUSED",
                }
            history_check = self._check_password_history(user_id=int(user_id), role=role, new_password=str(new_password or ""))
            if not history_check.get("success"):
                return history_check
            new_hash = _hash_password(str(new_password or ""))
            self._repo.update_password_hash(user_id=int(user_id), password_hash=new_hash)
            self._repo.add_password_history(user_id=int(user_id), password_hash=new_hash)
            self._repo.trim_password_history(
                user_id=int(user_id),
                keep_limit=self._password_history_limit(role),
            )
            self._repo.mark_first_login_completed(user_id=int(user_id))
            self._repo.reset_login_attempts(user_id=int(user_id))
    ...
    def verify_and_reset_password(self, *, username: str, answers: list[Any], new_password: str) -> dict[str, Any]:
        ...
        try:
            ...
            history_check = self._check_password_history(
                user_id=int(user["id"]),
                role=role,
    ...
    def get_security_questions(self, *, user_id: int) -> dict[str, Any]:
        try:
            ...
            if not user:
                return {"success": False, "error": "用户不存在", "code": "USER_NOT_FOUND"}
            rows = self._repo.list_security_questions(user_id=int(user_id))
    ...
    def set_security_questions(self, *, user_id: int, questions: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            if not user:
                return {"success": False, "error": "用户不存在", "code": "USER_NOT_FOUND"}
            self._repo.replace_security_questions(user_id=int(user_id), items=items)
            self._repo.set_security_setup_required(user_id=int(user_id), required=False)
