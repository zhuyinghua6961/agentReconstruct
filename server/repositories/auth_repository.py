"""Persistence helpers for auth and account security flows."""
...
class AuthRepository:
    ...
    def _execute_update(self, query: str, params: tuple[Any, ...] = ()) -> int:
        return int(execute_update(query, params))
    ...
    def has_column(self, column_name: str) -> bool:
        return str(column_name or "").strip() in self._columns()

    def has_table(self, table_name: str) -> bool:
        return str(table_name or "").strip() in self._tables()
    ...
    def get_by_username(self, username: str) -> dict[str, Any] | None:
        rows = self._execute_query(
            f"""
            SELECT {self._select_user_fields(include_password=True)}
            FROM users
            WHERE username = %s
            LIMIT 1
            """,
    ...
    def create_user(
        ...
        *,
        ...
        values: list[Any] = [str(username), str(password_hash), str(role), "active"]
    ...
    def increment_login_attempts(self, *, user_id: int, lock_threshold: int, lock_minutes: int) -> dict[str, Any]:
        ...
        if self.has_column("locked_until") and next_count >= int(lock_threshold):
            self._execute_update(
                ...
                WHERE id = %s
                """,
                (int(next_count), int(lock_minutes), int(user_id)),
            )
        else:
            self._execute_update(
                """
                ...
                WHERE id = %s
                """,
                (int(next_count), int(user_id)),
            )
        latest = self.get_by_id(int(user_id)) or {}
        return {
    ...
    def list_users(self, *, offset: int, limit: int) -> list[dict[str, Any]]:
        return self._execute_query(
            f"""
            SELECT {self._select_user_fields(include_password=False)}
            FROM users
            ORDER BY id DESC
            LIMIT %s OFFSET %s
            """,
    ...
    def trim_password_history(self, *, user_id: int, keep_limit: int) -> int:
        ...
        return self._execute_update(
            ...
            """,
            (int(user_id), int(user_id), int(keep_limit)),
        )
    ...
    def replace_security_questions(self, *, user_id: int, items: list[dict[str, Any]]) -> None:
        if not self.has_table("user_security_questions"):
            return
        self._execute_update("DELETE FROM user_security_questions WHERE user_id = %s", (int(user_id),))
        for item in items:
            self._execute_update(
                """
