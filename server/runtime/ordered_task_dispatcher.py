"""Ordered background task dispatcher."""
...
class OrderedTaskDispatcher:
    """Run keyed tasks in background while preserving FIFO order per key."""

    def __init__(self, *, max_workers: int = 4, logger: Any = None) -> None:
        self._executor = ThreadPoolExecutor(
            max_workers=max(1, int(max_workers)),
            thread_name_prefix="ordered-bg",
        )
            ...
            def _runner():
                if prev_future is not None:
                    try:
                        ...
                        # Previous task failure should not block later tasks.
                        pass
