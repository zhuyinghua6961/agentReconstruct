...
def test_require_auth_context_rejects_active_account_bound_to_disabled_department(monkeypatch):
    monkeypatch.setattr(auth_service_module.auth_service, "decode_token", lambda token: {"user_id": 7, "role": "user"})
    monkeypatch.setattr(
        auth_service_module.auth_service,
        "get_user_by_id",
    ...
    monkeypatch.setattr(
        auth_service_module.auth_service,
        "build_disabled_department_login_error",
        lambda user: {
            "success": False,
            ...
            "data": {
                "personnel": {
                    "employee_no": "T2024001",
                    "full_name": "张三",
                    "department_display": "磷酸铁锂事业部 / 材料研发部（已停用）",
                }
            },
        },
        raising=False,
    ...
    assert exc_info.value.code == "DEPARTMENT_DISABLED"
    assert exc_info.value.status_code == 403
    assert exc_info.value.extra_payload["data"]["personnel"]["department_display"].endswith("（已停用）")
...
def test_auth_service_login_rejects_disabled_department_after_password_and_personnel_checks():
    class FakeRepo:
        password_hash = _hash_password("Secret123!")
        reset_called = False
    ...
    class FakeDepartments:
        def describe_user_department(self, **kwargs):
            return {
                "primary_department_id": 1,
                "primary_department_name": "磷酸铁锂事业部",
                "primary_department_status": "disabled",
                "secondary_department_id": 11,
                "secondary_department_name": "材料研发部",
                "secondary_department_status": "active",
                "tertiary_department_id": None,
                "tertiary_department_name": None,
                "tertiary_department_status": None,
    ...
    assert result["http_status"] == 403
    assert service.status_code_for(result, ok_status=200) == 403
    assert result["error"] == "账号所属部门已停用，请联系管理员"
    assert result["data"]["personnel"] == {
        "employee_no": "T2024001",
        "full_name": "张三",
        "department_display": "磷酸铁锂事业部 / 材料研发部（已停用）",
...
def test_auth_service_login_does_not_disclose_disabled_department_when_password_is_wrong():
    class FakeRepo:
        password_hash = _hash_password("Secret123!")
...
def test_auth_service_login_allows_admin_bound_to_disabled_department():
    class FakeRepo:
        password_hash = _hash_password("Secret123!")
    ...
    class FakeDepartments:
        def describe_user_department(self, **kwargs):
            return {
                ...
                "tertiary_department_name": None,
                "department_effective_status": "disabled",
                "department_display": "磷酸铁锂事业部（已停用）",
    ...
    class FakePersonnel:
        ...
        def get_personnel_by_id(self, *, personnel_id: int | None):
            raise AssertionError("admin should not be blocked by department status")
