    ...
    if isinstance(value, dict) and value.get("code"):
        return UpstreamCallError(
            ...
            extra={
                key: value[key]
                for key in value
                if key not in {"code", "error", "component", "stage", "failure_stage", "status_code", "message", "retriable"}
            },
        )
    return None


def looks_like_embedding_failure(error_text: str) -> bool:
    lowered = str(error_text or "").strip().lower()
    return any(token in lowered for token in ("embedding", "embeddings", "embed api", "text-embedding"))
