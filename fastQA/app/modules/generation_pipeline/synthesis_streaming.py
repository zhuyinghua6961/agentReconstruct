...
- **语境**：始终以用户问题的对象与限定为主轴叙述；不要为了篇幅引入与回答问题关系不明的大段相邻综述分支。
- **术语**：若正文可能涉及**多套定义、不同测试层级或名称相近而不同含义**的量，须在基于卡片展开时逐项写清所指（以卡片用词为准）。
- **因果**：卡片仅支持相关性或单侧结果时，不要写成决定性「导致」「必然改善」或无条件跨条件「优于」。推断句不接单独 `(doi=…)`，且不突破卡片给的 not_allowed。
- **粉末振实密度 vs 电极压实密度**：二者不得在无换算或未在同一卡片中共现时互换结论或数值；卡片仅有振实密度时，不得写出压实密度的具体数值。
...
5. **禁止紧邻重复标注同一文献**：不得在「一句末尾已挂 `(doi=xxx)`」后，句号/换行紧随的下一句**再次**挂**完全相同**的 `(doi=xxx)`（包括小标题下方首句重复上一段的同一 DOI）。同一篇文献顺延的多句论述，请写成连贯的一段，只在**段末或小节末尾保留一处** `(doi=…)`，中间用「该研究还表明…」「进一步…」等承接且**勿再写**同名 DOI；只有换到新证据、新论断时再挂标注。
"""
...
- **语境**：以用户问题的对象与限定为主线；避免用泛化改性/性能综述替代对用户问题的针对性回答。
- **术语**：易出现混淆或名称相近而不同含义的表述，须在正文中分项说明各自适用场景（仅以原文为准）。
- **因果**：仅在原文明确对照或机理时写法可偏强；否则用「报告」「观察到」「关联」类措辞，禁止把拼装的多条证据揉成单句无条件强因果。
- **粉末振实密度 vs 电极压实密度**：二者不得在无换算或未在同一句证据中共现时互换结论或数值；原文仅有振实密度时，不得写出压实密度的具体数值。
...
5. **禁止紧邻重复标注同一文献**：不得在已挂 `(doi=xxx)` 的句子之后，紧贴的下一句再一次挂**相同** `(doi=xxx)`（含引言末与小节标题后首句的回贴）。顺延同一出处时只在**段末**保留一处 DOI。
"""
...
5. **禁止紧邻两句挂同一 `(doi=xxx)`**；顺延同文献时仅在段尾保留一处。
6. 输出 Markdown，禁止文末单独 DOI 列表。
...
## 语境、术语与因果（通用护栏，适用于任意用户问题）
- **语境**：始终围绕用户问题的对象与限定条件组织段落；若需旁涉相邻主题，须用一两句说明其与所答问题的关系，**禁止**把通用综述替代成对用户问题的直接回应。
- **术语与指标**：凡答案中可能同时出现**易混或不同层级**的概念（不同测试、不同形态/工段、名称相近而定义不同的量），必须分别写清指称对象与适用场景，**禁止**用同一含糊词指代多种不同含义。
- **因果措辞**：仅当证据原文给出机制、对照实验或明确结论时，允许使用「导致」「显著提升」「优于」等强因果或比较表述；若证据仅为共现、单点数据或机制未展开，改用「观察到」「在…条件下报告」「与…相关」等；**不得**把专家初稿或常识推理写成带 `(doi=…)` 的强因果结论，也**不得**把多文献碎片拼成一句无条件的绝对化因果链。
- **粉末振实密度 vs 电极压实密度**：二者物理含义不同。**禁止**用文献报告的振实密度数值或区间直接断言电极压实密度或「辊压后压实已达某水平」，除非证据原文在同一语境明确给出压实密度或等价表述。**禁止**在带 `(doi=…)` 的句子中把振实密度改写为压实密度叙述。
- **数值与外推**：凡带 `(doi=…)` 且含具体数字（范围、%、粒径、密度单位等）的句子，必须与对应证据片段逐句可追溯一致；**禁止**补全、插值或跨文献拼接成文献未给出的数值结论；趋势性推论不写 `(doi=…)` 或与证据拆开表述。
...
- 错误：在答案最后统一列出所有DOI
- 错误：一句话引用多个DOI
- **错误**：紧邻的两句分别写**完全相同**的两个 `(doi=…)`（含段末与小标题后首句重复）；顺延同一文献时仅保留一处 DOI。
{fact_mode_rules}
"""
