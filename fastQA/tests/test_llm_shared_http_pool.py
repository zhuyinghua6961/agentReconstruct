...
def test_shared_pool_reads_keepalive_and_capacity_from_env(monkeypatch):
    fake_httpx = _FakeHttpx()
...
def test_shared_pool_config_warns_when_env_values_are_invalid(monkeypatch, caplog):
    monkeypatch.setenv("FASTQA_LLM_HTTP_POOL_TIMEOUT_SECONDS", "not-a-number")
    monkeypatch.setenv("FASTQA_LLM_HTTP_MAX_CONNECTIONS", "bad-value")
    ...
    assert config.pool_timeout_seconds == 30.0
    assert config.max_connections == 160
    assert "FASTQA_LLM_HTTP_POOL_TIMEOUT_SECONDS" in caplog.text
    assert "FASTQA_LLM_HTTP_MAX_CONNECTIONS" in caplog.text
