...
_SERVICE_LABEL = "patent"
_LOGGER = logging.getLogger("patent.object_reader")
...
def _default_runtime_root() -> Path:
    configured = str(os.getenv("PATENT_SERVICE_RUNTIME_ROOT") or os.getenv("PATENT_OBJECT_CACHE_DIR") or "").strip()
    base = Path(configured).expanduser() if configured else Path(tempfile.gettempdir()) / "patent-object-cache"
...
class ObjectReader:
    ...
    def _storage_ref_for_object(self, object_name: str, *, bucket: str | None = None) -> str:
        normalized_bucket = str(bucket or os.getenv("MINIO_BUCKET") or "agentcode").strip() or "agentcode"
        normalized_object_name = str(object_name or "").strip().lstrip("/")
        if not normalized_object_name:
            raise ObjectReaderProtocolError("object_name_missing")
        return f"minio://{normalized_bucket}/{normalized_object_name}"
    ...
    def stat_object(self, object_name: str, *, bucket: str | None = None) -> ObjectStat:
        return self.stat(self._storage_ref_for_object(object_name, bucket=bucket))
    ...
    def read_object_bytes(self, object_name: str, *, bucket: str | None = None) -> bytes:
        return self.read_bytes(self._storage_ref_for_object(object_name, bucket=bucket))

    def read_json(self, storage_ref: str) -> Any:
        return json.loads(self.read_bytes(storage_ref).decode("utf-8"))

    def read_object_json(self, object_name: str, *, bucket: str | None = None) -> Any:
        return self.read_json(self._storage_ref_for_object(object_name, bucket=bucket))
