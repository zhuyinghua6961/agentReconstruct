...
def test_stage2_runtime_signature_includes_behavior_affecting_fields(monkeypatch):
    ...
    assert signature["stage2_rerank_endpoint_family"] == "dashscope"
...
def test_stage2_runtime_signature_ignores_retired_rerank_endpoint_aliases(monkeypatch):
    ...
    assert signature["stage2_rerank_provider"] == "none"
    assert signature["stage2_rerank_model"] == ""
    assert signature["stage2_rerank_base_url"] == ""
    assert signature["stage2_rerank_timeout_seconds"] == 20.0
    assert signature["stage2_rerank_endpoint_family"] == "none"
