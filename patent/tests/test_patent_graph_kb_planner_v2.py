...
def test_planner_v2_returns_template_plan_for_legacy_question():
    ...
    assert plan.legacy_template_plan is None
    assert plan.diagnostics["matched_rule"] == "legacy_template"
    assert plan.diagnostics["strategy"] == "parametric"
    assert [item["path_id"] for item in plan.parametric_slots["candidate_queries"]][:2] == [
...
def test_planner_v2_returns_parametric_plan_for_inventor_listing():
    ...
    assert plan is not None
    assert plan.strategy == "parametric"
    assert plan.intent == "list_patents_by_inventor"
...
def test_parametric_query_builders_cover_phase2_families():
    ...
    assert [item["path_id"] for item in candidates] == ["list_patents_by_ipc_code_prefix"]
    assert candidates[0]["params"]["ipc_code_prefix"] == "H01M10"
    assert "STARTS WITH $ipc_code_prefix" in candidates[0]["cypher"]

    candidates = build_patent_parametric_query_candidates("CN100355122C 使用了哪些气氛条件？")
    assert [item["path_id"] for item in candidates][:2] == ["list_patent_atmospheres", "lookup_patent_by_id"]
    ...
    assert [item["path_id"] for item in candidates][:2] == ["list_patent_embodiment_insights", "lookup_patent_by_id"]
    assert candidates[0]["params"]["patent_id"] == "CN100355122C"
    assert " AS insight_conclusion" in candidates[0]["cypher"]

    candidates = build_patent_parametric_query_candidates("H01M10/0525 有多少专利？")
    assert [item["path_id"] for item in candidates] == ["count_patents_by_ipc_full_code"]
    assert candidates[0]["params"]["ipc_full_code"] == "H01M10/0525"
    ...
    assert candidates[0]["params"]["applicant_name"] == "宁德时代新能源科技股份有限公司"
...
def test_parametric_query_builders_cover_compare_safe_families():
    ...
    assert candidates[0]["params"]["patent_ids"] == ["CN100355122C", "CN100371239C"]
    assert "material_options" in candidates[0]["cypher"]
    ...
    assert candidates[0]["params"]["patent_ids"] == ["CN100355122C", "CN100371239C"]
    assert " AS solutions" in candidates[0]["cypher"]


def test_planner_v2_distinguishes_ipc_prefix_code_prefix_and_full_code():
    registry = build_default_patent_schema_registry()
    decision = PatentGraphSemanticDecision(mode="direct_answer", route_family="precise", diagnostics={"matched_rule": "ipc"})

    plan = build_patent_graph_query_plan_v2(question="H01M 下有哪些专利？", decision=decision, schema_registry=registry)
    assert [item["path_id"] for item in plan.parametric_slots["candidate_queries"]] == ["list_patents_by_ipc_prefix"]

    plan = build_patent_graph_query_plan_v2(question="H01M10 下有哪些专利？", decision=decision, schema_registry=registry)
    assert [item["path_id"] for item in plan.parametric_slots["candidate_queries"]] == ["list_patents_by_ipc_code_prefix"]

    plan = build_patent_graph_query_plan_v2(question="H01M10/0525 下有哪些专利？", decision=decision, schema_registry=registry)
    assert [item["path_id"] for item in plan.parametric_slots["candidate_queries"]] == ["list_patents_by_ipc_full_code"]
