...
<template>
  <section class="quota-panel">
    <div class="panel-header">
      <div>
        <h2>配额管理</h2>
        <p>维护管理员可见的标准配额配置，当前统一在管理员后台内切换查看。</p>
      </div>
      <button class="btn-create" :disabled="loading" @click="startCreate">新增配额</button>
    </div>
    ...
    <div v-else class="config-list">
      <div v-for="config in configs" :key="config.quota_type" class="config-item">
        ...
        <div class="config-actions">
          <button class="btn-edit" @click="startEdit(config)">编辑配置</button>
        </div>
      </div>
    </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="form-group">
          <label class="checkbox-label">
            <input v-model="creatingConfig.is_active" type="checkbox">
            <span>启用此配额</span>
          </label>
        </div>
    ...
    <div v-if="editingConfig" class="modal-overlay" @click="cancelEdit">
      <div class="modal-content" @click.stop>
        ...
        <div class="form-group">
          <label class="checkbox-label">
            <input v-model="editingConfig.is_active" type="checkbox">
            <span>启用此配额</span>
          </label>
        </div>
