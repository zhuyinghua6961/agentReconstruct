...
test('PdfReader full document translation exposes download after stream completion', () => {
  assert.match(source, /const fullDocumentTranslationCompleted = ref\(false\)/)
  assert.match(source, /const fullDocumentTranslationTruncated = ref\(false\)/)
  assert.match(source, /const canDownloadFullDocumentTranslation = computed\(\(\) =>/)
  assert.match(source, /const fullDocumentTranslationDownloadLabel = computed\(\(\) =>/)
  assert.match(source, /fullDocumentTranslationCompleted\.value/)
  assert.match(source, /fullDocumentTranslationTruncated\.value/)
  assert.match(source, /function downloadFullDocumentTranslation\(\)/)
  assert.match(source, /import \{ downloadTextFile \} from '\.\.\/utils\/downloadTextFile\.js'/)
  assert.match(source, /import \{ buildFullDocumentTranslationFilename \} from '\.\.\/utils\/pdfReaderTranslationDownload\.js'/)
  assert.match(source, /class="translation-document-button-row"/)
  assert.match(source, /fullDocumentTranslationDownloadLabel/)
  assert.match(source, /结构化 Markdown 译文/)
  assert.match(source, /下载已译部分/)
  assert.match(source, /仅翻译了部分内容/)
  assert.match(source, /:disabled="!canDownloadFullDocumentTranslation"/)
  assert.match(source, /@click="downloadFullDocumentTranslation"/)
  assert.match(source, /if \(eventType === 'done'\)[\s\S]*?fullDocumentTranslationCompleted\.value = true/)
  assert.match(source, /if \(eventType === 'error'\)[\s\S]*?fullDocumentTranslationCompleted\.value = false/)
  assert.match(source, /function resetDocumentTranslationState\(\)[\s\S]*?fullDocumentTranslationCompleted\.value = false/)
  assert.match(source, /fullDocumentTranslationCompleted\.value = false[\s\S]*?const translatedSegments = \[\]/)
})
