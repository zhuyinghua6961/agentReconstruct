...
class ConversationJsonStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._project_root = Path(project_root or Path(__file__).resolve().parents[3]).resolve()
        self._logger = logger
        self._storage_backend = storage_backend
        self._locks_guard = Lock()
        self._locks: dict[str, Lock] = {}

        raw_base = str(os.getenv("CHAT_JSON_BASE_DIR", "data/conversations")).strip() or "data/conversations"
        base_path = Path(raw_base)
        if not base_path.is_absolute():
            base_path = (self._project_root / base_path).resolve()
